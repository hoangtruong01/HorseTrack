...
export function StatCard({
  ...
  return (
    <article
      className={cn(
        "group relative overflow-hidden rounded-xl border border-border bg-card p-4 shadow-lg transition duration-200 hover:-translate-y-0.5 hover:border-border/80 sm:p-5",
        className,
      )}
    >
      <div
      ...
      {trend ? (
        <p className="mt-3 inline-flex rounded-full border border-border bg-muted/30 px-2.5 py-1 text-xs font-bold uppercase tracking-[0.14em] text-foreground/80">
          {trend}
        </p>
      ) : null}
    </article>
