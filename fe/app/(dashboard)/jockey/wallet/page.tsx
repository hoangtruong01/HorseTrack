      ...
      {isLoading && transactions.length === 0 ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Tổng thưởng nài ngựa</p>
              <p className="mt-1 font-mono text-xl font-black text-emerald-400">{(stats?.totalPoints ?? 0).toLocaleString("vi-VN")} PTS</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Tỷ lệ thắng trận</p>
              <p className="mt-1 font-mono text-xl font-black text-amber-500">{stats?.races?.winRate ?? 0}%</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Số trận đã tham gia</p>
              <p className="mt-1 font-mono text-xl font-black text-primary">{stats?.races?.participated ?? 0}</p>
            </div>
          </div>
