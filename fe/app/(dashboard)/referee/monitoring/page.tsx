...
import { Activity, ArrowRight, Clock, Trophy } from "lucide-react";
      ...
      try {
        ...
        const list = (result.data || []).filter(
          (a) =>
            ...
            a.raceId !== null &&
            (a.raceId.status === "READY" || a.raceId.status === "LIVE" || a.raceId.status === "CHECKING")
        );
        setAssignments(list);
      } catch (err) {
  ...
  return (
    <main className="space-y-6 max-w-6xl mx-auto px-4 sm:px-6">
      <PageHeader
        ...
        title="Race Monitoring (Giám sát cuộc đua)"
        description="Chọn một cuộc đua đang diễn ra hoặc đã sẵn sàng xuất phát dưới đây để theo dõi live camera, ghi nhận vi phạm và xử lý sự cố trực tiếp."
      />
      ...
      {assignments.length === 0 ? (
        <section className="flex flex-col items-center justify-center text-center p-12 rounded-2xl border border-dashed border-border bg-muted/30 max-w-lg mx-auto space-y-3">
          ...
          <h4 className="font-bold text-foreground uppercase text-sm">Không có cuộc đua đang giám sát</h4>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Chỉ hiển thị các cuộc đua đang ở trạng thái Sẵn sàng xuất phát (READY) hoặc Đang chạy (LIVE).
          </p>
        </section>
      ) : (
