...
export default function AdminPrizesPage() {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    ...
    try {
      ...
      showToast(t("pages.admin.prizes.toastStatus", { status }));
  ...
  return (
    <main className="space-y-6">
      ...
      <TransHtml
        ...
        i18nKey="pages.admin.prizes.total"
      ...
      <div className="rounded-2xl border dark:border-white/10 border-border dark:bg-[#15151E]/85 bg-card overflow-hidden">
        {loading ? (
          ...
          <div className="flex items-center justify-center py-16 text-muted-foreground text-sm">{t("pages.admin.prizes.empty")}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b dark:border-white/10 border-border">
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("pages.admin.prizes.colRecipient")}</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("pages.admin.prizes.colRace")}</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("pages.admin.prizes.colHorse")}</th>
                  <th className="px-5 py-3.5 text-center text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("pages.admin.prizes.colPoints")}</th>
                  ...
                  <th className="px-5 py-3.5 text-right text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("pages.admin.prizes.colAction")}</th>
                </tr>
              </thead>
                  ...
                  <tr key={p._id} className="hover:dark:bg-white/[0.02] bg-muted/50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-right">
                      {p.status === "PENDING" && (
                        ...
                        >
                          {t("pages.admin.prizes.markPaid")}
                        </button>
                      )}
                      {p.status === "PAID" && (
                        <span className="text-xs text-muted-foreground">
                          {t("pages.admin.prizes.paidOn", {
