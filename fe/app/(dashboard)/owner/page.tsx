...
export function OwnerDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      setIsLoading(true);
      try {
        const [statsRes, profileRes, invRes, regRes, horsesRes] = await Promise.all([
  ...
  // Process data for UI
  const recentInvs = [...invitations].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()).slice(0, 4);
  const upcomingRaces = registrations.filter(r => r.status === "APPROVED").slice(0, 3);
  const sickHorses = horses.filter(h => h.healthStatus === "SICK" || h.healthStatus === "INJURED");
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-3">
            <Button asChild className="rounded-full bg-[#E10600] hover:bg-[#B80500] text-white font-bold transition">
              <Link href="/owner/horses/new">
                Thêm chiến mã
                <PlusCircle className="size-4 ml-1.5" />
              </Link>
            </Button>
            <Button asChild variant="outline" className="rounded-full border-border hover:bg-muted text-foreground font-bold transition">
          ...
          <div className="lg:w-1/3 rounded-2xl border border-border bg-card p-5 shadow-sm relative overflow-hidden flex flex-col justify-center">
            ...
            {isLoading ? (
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                <div>
                  <h2 className="text-lg font-black uppercase text-foreground">{profile?.fullName}</h2>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 flex flex-col h-full shadow-sm">
            ...
            <div className="flex-1">
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {recentInvs.map((inv) => (
                    <div key={inv.id || inv._id} className="p-3 rounded-xl border border-border bg-muted/20 flex justify-between items-center hover:bg-muted/40 transition">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 text-xs">
                          <span className="font-bold text-foreground">
                            {typeof inv.jockeyUserId === 'object' ? (inv.jockeyUserId as any)?.fullName : "Jockey"}
                          </span>
                        ...
                        <span className="text-[10px] text-foreground font-bold uppercase truncate max-w-[200px]" title={inv.raceId?.name}>
                          {inv.raceId?.name}
                        </span>
                      </div>
                      
                      <div className="shrink-0 pl-2">
                        {inv.status === "PENDING" && <StatusBadge label="Đang chờ" tone="yellow" />}
                        {inv.status === "ACCEPTED" && <StatusBadge label="Đã nhận" tone="green" />}
                        {inv.status === "REJECTED" && <StatusBadge label="Từ chối" tone="red" />}
                        {inv.status === "CANCELLED" && <StatusBadge label="Đã hủy" tone="slate" />}
                        {inv.status === "EXPIRED" && <StatusBadge label="Hết hạn" tone="slate" />}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {!isLoading && sickHorses.length > 0 && (
              <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 shadow-sm flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-black text-red-500 uppercase">Cảnh báo Sức khỏe Chiến mã</h4>
                  <p className="text-xs text-red-500/80 mt-1">
                    Bạn có {sickHorses.length} chiến mã đang không khỏe mạnh ({sickHorses.map(h => h.name).join(", ")}). Vui lòng kiểm tra tab Chiến mã.
                  </p>
                </div>
                <Button 
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-3">
                    {upcomingRaces.map((reg) => (
                      <div key={reg.id || reg._id} className="relative overflow-hidden rounded-xl border border-border bg-muted/20 p-3 hover:border-teal-500/30 transition">
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-teal-500" />
                        <div className="pl-2 flex flex-col gap-2">
                          <h4 className="text-xs font-black uppercase text-foreground truncate" title={reg.raceId?.name}>
                            {reg.raceId?.name}
                          </h4>
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-bold flex items-center gap-1.5 text-foreground bg-background border border-border px-2 py-0.5 rounded-md">
                              <Sparkles className="size-3 text-primary" /> {reg.horseId?.name}
                            </span>
                          </div>
                        </div>
                      </div>
