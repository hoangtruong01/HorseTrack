...
import {
  ...
  type Registration,
} from "@/features/registrations/components/owner-registration-table";
import { racesApi, tournamentsApi } from "@/lib/api-client";
import {
  ArrowRight,
  Award,
  ChevronRight,
...
type Race = {
  ...
  status: string;
  tournamentId:
    | { _id: string; name: string; startDate?: string; endDate?: string }
    | string;
};

const tournamentStatusLabel: Record<
  string,
  { label: string; tone: "green" | "yellow" | "red" | "slate" | "teal" }
> = {
...
export default function OwnerRacesBrowserPage() {
      ...
      if (response.ok) {
        ...
        if (resData.success) {
          ...
          const mapped: Registration[] = (
            ...
            const tournamentId = item.tournamentId as
              | Record<string, unknown>
              | null
              | undefined;
            const raceId = item.raceId as
              | Record<string, unknown>
              | null
              | undefined;
            const horseId = item.horseId as
              | Record<string, unknown>
              | null
              | undefined;
            const ownerId = item.ownerId as
              | Record<string, unknown>
              | string
              | null
              | undefined;
            return {
              ...
              tournamentId:
                ((tournamentId?._id || tournamentId?.id) as string) || "",
              tournamentName:
                (tournamentId?.name as string) || "Giải đấu tự do",
              ...
              ownerId:
                (typeof ownerId === "object" && ownerId !== null
                  ? ((ownerId._id || ownerId.id) as string)
                  : (ownerId as string)) || "",
  ...
  const fetchRaces = useCallback(async (tournamentId: string) => {
    ...
    try {
      const data = await racesApi.listByTournament(tournamentId, {
        limit: 100,
      });
      setRaces(data.data || []);
    } catch {
    ...
    <main className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      {activeTab === "browse" ? (
          ...
          <aside className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-full p-1.5 hover:bg-muted text-muted-foreground/60 hover:text-foreground transition"
              >
            ...
            {loadingTournaments ? (
              ...
              <div className="space-y-2 max-h-[calc(100vh-320px)] overflow-y-auto pr-1 custom-scrollbar">
                {filteredTournaments.map((t) => {
                  ...
                  const sInfo = tournamentStatusLabel[t.status] || {
                    label: t.status,
                    tone: "slate" as const,
                  };
                  ...
                  return (
                    <button
                      key={tid}
                      onClick={() =>
                        setSelectedTournamentId(isSelected ? null : tid)
                      }
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h4
                          className={`text-sm font-black uppercase tracking-tight leading-tight transition ${isSelected ? "text-primary" : "text-foreground group-hover:text-primary"}`}
                        >
                          {t.name}
                        </h4>
                        <ChevronRight
                          className={`size-4 shrink-0 transition ${isSelected ? "text-primary rotate-90" : "text-muted-foreground/40 group-hover:text-muted-foreground/60"}`}
                        />
                      </div>
                      ...
                      {t.prizePool != null && (
                        <div className="mt-2 text-[10px] text-muted-foreground/60">
                          ...
                          <strong className="text-teal-400 font-mono">
                            {t.prizePool.toLocaleString()} điểm
                          </strong>
                        </div>
                      )}
                    </button>
          ...
          <section className="space-y-4">
            {!selectedTournamentId ? (
              <div className="rounded-2xl border border-dashed border-border bg-card/40 p-16 text-center">
                ...
                <h3 className="text-lg font-black text-muted-foreground/60 uppercase tracking-tight mb-2">
                  Chọn một giải đấu
                </h3>
                <p className="text-xs text-foreground/25 max-w-sm mx-auto">
                  Chọn giải đấu chính từ bảng bên trái để xem danh sách các vòng
                  đua nhỏ và đăng ký chiến mã tham gia.
                </p>
              </div>
            ) : (
              <>
                  ...
                  <div className="rounded-2xl border border-border bg-card/85 p-5 relative overflow-hidden">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <h3 className="text-xl font-black uppercase text-foreground tracking-tight mt-1">
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-4 text-xs text-muted-foreground border-t border-border pt-3">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-teal-400 font-mono text-[11px]">
                          {selectedTournament.prizePool?.toLocaleString() || 0}{" "}
                          điểm
                        </span>
                      </div>
                      <div>
                ...
                {loadingRaces ? (
                  ...
                  <div className="rounded-2xl border border-border bg-card/60 p-12 text-center shadow-xl">
                    ...
                    <p className="text-xs text-foreground/55 max-w-sm mx-auto">
                      Giải đấu này chưa có vòng đua nào được lên lịch. Vui lòng
                      quay lại sau!
                    </p>
                  </div>
                ) : (
                          ...
                          <div>
                            ...
                            <div className="mt-4 grid gap-2 text-xs text-muted-foreground">
                              ...
                              <span className="inline-flex items-center gap-2">
                                ...
                                Cự ly: {race.distanceMeters}m
                              </span>
                              <span className="inline-flex items-center gap-2">
                                <Users className="size-4 text-primary shrink-0" />
                                Số lượng: {race.participantsCount || 0}/
                                {race.maxParticipants || 20} chiến mã
                              </span>
                              {race.prize != null && (
                                <span className="inline-flex items-center gap-2">
                                  ...
                                  Giải thưởng:{" "}
                                  <strong className="text-teal-400 font-mono">
                                    {race.prize.toLocaleString()} điểm
                                  </strong>
                                </span>
                              )}
                            </div>
                          ...
                          <div className="mt-5 pt-4 border-t border-border flex gap-2">
                            {isOpenRegistration ? (
                              isFull ? (
                                <Button
                                  disabled
                                  className="rounded-xl flex-1 text-xs py-2 h-9 bg-muted text-muted-foreground/60 border border-border"
                                >
                                  Trận đấu đã đầy
                                </Button>
                              ) : (
                                <Button
                                  asChild
                                  className="rounded-xl flex-1 text-xs py-2 h-9 bg-primary hover:bg-primary/90 text-primary-foreground font-bold uppercase tracking-wider"
                                >
                                  <Link
                                    href={`/owner/races/${raceId}/register`}
                                  >
                                    Ghi danh ngay{" "}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black uppercase tracking-[0.15em] text-foreground/85">
                Yêu cầu đã gửi
              </h3>
              <p className="text-xs text-muted-foreground/60 mt-1">
                Danh sách chi tiết các hồ sơ đăng ký tham gia vòng đua đã được
                gửi.
              </p>
            </div>
            <Button
          ...
          {loadingRegistrations ? (
            <div className="flex flex-col items-center justify-center py-20 text-foreground/55">
              ...
              <p className="mt-4 text-xs font-mono uppercase tracking-widest">
                Đang tải danh sách gửi yêu cầu...
              </p>
            </div>
          ) : (
            <OwnerRegistrationTable
