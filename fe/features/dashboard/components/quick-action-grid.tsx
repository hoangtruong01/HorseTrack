...
const toneClassName: Record<QuickAction["tone"], string> = {
  ...
  neutral: "dark:border-white/15 border-border dark:bg-white/[0.04] bg-muted/50 dark:text-white text-foreground",
};
        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              key={action.title}
              href={action.href}
              className="group rounded-xl border dark:border-white/10 border-border dark:bg-white/[0.03] bg-muted/50 p-4 transition duration-200 hover:-translate-y-0.5 hover:border-primary/45 hover:dark:bg-white/[0.06] bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <ArrowUpRight className="size-4 text-muted-foreground transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:dark:text-white text-foreground" />
              </div>
              <h3 className="mt-4 text-lg font-black uppercase tracking-tight dark:text-white text-foreground">
                {action.title}
              </h3>
