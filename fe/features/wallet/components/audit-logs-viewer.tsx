  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-lg">
      ...
      <div className="relative mt-6">
        <Terminal className="absolute top-1/2 left-4 size-4 -translate-y-1/2 text-muted-foreground/60" />
        <input
          type="text"
          placeholder="Filter audit entries by action, user or details..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="h-12 w-full rounded-full border border-border bg-muted/50 pl-11 pr-6 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary"
        />
      </div>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-border">
        {filteredLogs.length === 0 ? (
          ...
          <table className="min-w-[800px] w-full text-left text-sm">
            <thead className="bg-muted/60 text-xs font-black uppercase tracking-[0.15em] text-muted-foreground">
              <tr>
            ...
            <tbody className="divide-y divide-border bg-card font-mono text-xs">
              {filteredLogs.map((log) => {
                let badgeColor =
                  "border-border bg-muted text-muted-foreground dark:text-white/70 dark:bg-muted/5";
                if (
                  log.action.includes("APPROVED") ||
                  log.action.includes("PAID") ||
                  log.action.includes("PUBLISHED") ||
                  log.action.includes("PRIZE")
                ) {
                  badgeColor =
                    "border-emerald-600/30 bg-emerald-50 text-emerald-800 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-400";
                } else if (
                  ...
                  badgeColor =
                    "border-amber-500/30 bg-amber-50 text-amber-900 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400";
                }

                return (
                  <tr key={log.id} className="transition hover:bg-muted/[0.02]">
                    {/* Action */}
                    ...
                    {/* Details */}
                    <td className="px-4 py-4 font-semibold leading-relaxed max-w-sm whitespace-pre-wrap text-muted-foreground">
                      {log.details}
                    </td>

                    {/* IP Address */}
                    <td className="px-4 py-4 text-muted-foreground/60">
                      {log.ipAddress}
                    </td>
