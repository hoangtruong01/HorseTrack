    ...
    <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-[0_24px_64px_rgba(0,0,0,0.48)] sm:p-6">
      ...
      <div className="relative mt-5">
        ...
        <input
          ...
          className="h-12 w-full rounded-xl border border-border bg-muted/50 pl-11 pr-6 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary transition"
        />
      </div>
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-border bg-muted/30">
        {filteredTransactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            <Coins className="size-10 text-muted-foreground/30 animate-pulse" />
          ...
          <div className="w-full overflow-x-auto select-none">
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              <thead className="bg-muted/[0.03] text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground border-b border-border">
                <tr>
              ...
              <tbody className="divide-y divide-border">
                {filteredTransactions.map((tx) => {
                  const meta = typeMeta[tx.type] || { icon: Coins, color: "text-muted-foreground bg-muted border-border", label: "Giao dịch" };
                    ...
                    <tr key={tx.id} className="transition hover:bg-muted/[0.015]">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-black uppercase tracking-wider text-foreground text-[11px] sm:text-xs">
                              {meta.label}
                            </p>
                      ...
                      {/* Stacked Date & Time */}
                      <td className="px-4 py-3.5 whitespace-nowrap text-muted-foreground font-mono text-[11px] leading-relaxed">
