...
const fieldClass =
  "h-11 w-full rounded-xl border border-input bg-background pl-10 pr-4 text-sm text-foreground outline-none transition placeholder:text-muted-foreground focus:border-[#E10600] focus:ring-4 focus:ring-[#E10600]/15 disabled:cursor-not-allowed disabled:opacity-60";
const labelClass =
...
export function RegisterForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      customToast.success("Thành công!", "Đăng ký tài khoản thành công! Đang chuyển hướng...");
      window.location.href = selectedPreview.entryPath;
    } catch (err) {
      const errMsg = (err as Error).message || "Đăng ký tài khoản thất bại.";
      setErrorMsg(errMsg);
      customToast.error("Đăng ký thất bại", errMsg);
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <div className="pt-1">
        <label className="flex items-start gap-2.5 text-xs text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
          <input
            ...
            type="checkbox"
            required
            className="mt-0.5 size-4 shrink-0 rounded border-input bg-background accent-[#E10600] focus:ring-offset-0 focus:ring-0"
          />
      ...
      <button
        ...
        className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#E10600] text-sm font-black uppercase tracking-[0.16em] text-foreground hover:bg-[#B80500] hover:scale-[1.01] active:scale-[0.99] transition-all duration-150"
      >
        {isSubmitting ? (
          <>
